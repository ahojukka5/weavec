; generated by weavec
; source: test/performance/wir/0111_sum_odds_to_n_i32.wir
; core-version: 2

; function: main
; params: none
; returns: i32
define i32 @main() {
entry:
  ; return
  %t0 = call i32 @sum_odds_to(i32 15)
  ret i32 %t0
}

; function: sum_odds_to
; params: i32
; returns: i32
define i32 @sum_odds_to(i32 %n) {
entry:
  %v.addr = alloca i32
  %total.addr = alloca i32
  ; let v
  store i32 1, ptr %v.addr
  ; let total
  store i32 0, ptr %total.addr
  ; while condition
  br label %while.cond
while.cond:
  %t0 = load i32, ptr %v.addr
  %t1 = icmp sle i32 %t0, %n
  br i1 %t1, label %while.body, label %while.end
while.body:
  ; while body
  ; set total
  %t2 = load i32, ptr %total.addr
  %t3 = load i32, ptr %v.addr
  %t4 = add i32 %t2, %t3
  store i32 %t4, ptr %total.addr
  ; set v
  %t5 = load i32, ptr %v.addr
  %t6 = add i32 %t5, 2
  store i32 %t6, ptr %v.addr
  br label %while.cond
while.end:
  ; return
  %t7 = load i32, ptr %total.addr
  ret i32 %t7
}

